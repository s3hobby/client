...
type httpRequesterHandler[Input any, OutputBase any, OutputPtr interface {
	*OutputBase
	metadataHolder
}] struct{}
...
func (*httpRequesterHandler[Input, OutputBase, OutputPtr]) Handle(ctx context.Context, input *handlerInput[Input]) (*handlerOutput[OutputPtr], error) {
	...
	input.ServerRequest.Header.CopyTo(&metadata.RequestHeaders)
	output.ServerResponse.Header.CopyTo(&metadata.ResponseHeaders)
	output.CallOutput.setMetadata(&metadata)
...
func (*serverSideErrorMiddleware[Input, Output]) Middleware(ctx context.Context, input *handlerInput[Input], next Handler[Input, Output]) (*handlerOutput[Output], error) {
	...
	sse, err := NewServerSideError(input.OperationName, output.ServerResponse)
	if err != nil {
		return nil, err
	}
